'use client'
import '../style.css'
import Link from "next/link";
import React, {useEffect, useState} from "react";
import {useForm} from "react-hook-form";
import {registerRequest} from "@/services/requests/auth/register";
import {toast, Toaster} from "react-hot-toast";
import {config200, config400} from "@/services/toast/config";
import pic from  '@/public/f.png'
import {yupResolver} from "@hookform/resolvers/yup";
import {registerValidation} from "@/services/validations/validation";
import {BeatLoader} from "react-spinners";

const Page = () => {


    let {register,handleSubmit,reset,watch,
        formState: { errors }} = useForm({
        resolver: yupResolver(registerValidation),
        mode:"all"
    })
     let [prevImage,setPrevImage] = useState()
    let [loading, setLoading] = useState(false)
        let inputImage = watch('image')


               useEffect(()=>{
                   if (inputImage && inputImage.length > 0) {

                      let urlIMG= URL.createObjectURL(inputImage[0])
                        setPrevImage(urlIMG)

                   }
               },[inputImage])



    const onSubmit  = async (data) => {
        setLoading(true)

            let formData = new FormData();
            formData.append('name', data.name);
            formData.append('email', data.email);
            formData.append('password', data.password);
            formData.append('image', data.image[0]);


        let response   = await registerRequest(formData)
        if (response.status === 200) {
            setLoading(false)


            toast.success('ثبت نام با موفقیت ساخته شد',config200)
        } else {
            setLoading(false)
            toast.error(response,config400)
        }
    }

    return (
        <div className={'containerAuth'}>

            <div className="wrapper">
                <div className="title">
                    فرم ثبت نام
                </div>
                <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data" >
                    <div className="field">
                        <input {...register('name')} type="text" required />
                        <label>نام کاربری</label>
                    </div>

                    {
                        errors.name &&
                        <p className={'flex items-center my-5'}>
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#ff6467"/>
                                <path d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z" fill="#ff6467"/>
                                <path d="M12 16.9999C11.87 16.9999 11.74 16.9699 11.62 16.9199C11.5 16.8699 11.39 16.7999 11.29 16.7099C11.2 16.6099 11.13 16.5099 11.08 16.3799C11.03 16.2599 11 16.1299 11 15.9999C11 15.8699 11.03 15.7399 11.08 15.6199C11.13 15.4999 11.2 15.3899 11.29 15.2899C11.39 15.1999 11.5 15.1299 11.62 15.0799C11.86 14.9799 12.14 14.9799 12.38 15.0799C12.5 15.1299 12.61 15.1999 12.71 15.2899C12.8 15.3899 12.87 15.4999 12.92 15.6199C12.97 15.7399 13 15.8699 13 15.9999C13 16.1299 12.97 16.2599 12.92 16.3799C12.87 16.5099 12.8 16.6099 12.71 16.7099C12.61 16.7999 12.5 16.8699 12.38 16.9199C12.26 16.9699 12.13 16.9999 12 16.9999Z" fill="#ff6467"/>
                            </svg>
                            <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.name.message}</span>
                        </p>
                    }

                    <div className="field">
                        <input {...register('email')} type="text" required/>
                        <label>ادرس ایمیل</label>
                    </div>
                    {
                        errors.email &&
                        <p className={'flex items-center my-5'}>
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#ff6467"/>
                                <path d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z" fill="#ff6467"/>
                                <path d="M12 16.9999C11.87 16.9999 11.74 16.9699 11.62 16.9199C11.5 16.8699 11.39 16.7999 11.29 16.7099C11.2 16.6099 11.13 16.5099 11.08 16.3799C11.03 16.2599 11 16.1299 11 15.9999C11 15.8699 11.03 15.7399 11.08 15.6199C11.13 15.4999 11.2 15.3899 11.29 15.2899C11.39 15.1999 11.5 15.1299 11.62 15.0799C11.86 14.9799 12.14 14.9799 12.38 15.0799C12.5 15.1299 12.61 15.1999 12.71 15.2899C12.8 15.3899 12.87 15.4999 12.92 15.6199C12.97 15.7399 13 15.8699 13 15.9999C13 16.1299 12.97 16.2599 12.92 16.3799C12.87 16.5099 12.8 16.6099 12.71 16.7099C12.61 16.7999 12.5 16.8699 12.38 16.9199C12.26 16.9699 12.13 16.9999 12 16.9999Z" fill="#ff6467"/>
                            </svg>
                            <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.email.message}</span>
                        </p>
                    }
                    <div className="field">
                        <input {...register('password')} type="password" required/>
                        <label >رمز ورود</label>
                    </div>
                    {
                        errors.password &&
                        <p className={'flex items-center my-5'}>
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#ff6467"/>
                                <path d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z" fill="#ff6467"/>
                                <path d="M12 16.9999C11.87 16.9999 11.74 16.9699 11.62 16.9199C11.5 16.8699 11.39 16.7999 11.29 16.7099C11.2 16.6099 11.13 16.5099 11.08 16.3799C11.03 16.2599 11 16.1299 11 15.9999C11 15.8699 11.03 15.7399 11.08 15.6199C11.13 15.4999 11.2 15.3899 11.29 15.2899C11.39 15.1999 11.5 15.1299 11.62 15.0799C11.86 14.9799 12.14 14.9799 12.38 15.0799C12.5 15.1299 12.61 15.1999 12.71 15.2899C12.8 15.3899 12.87 15.4999 12.92 15.6199C12.97 15.7399 13 15.8699 13 15.9999C13 16.1299 12.97 16.2599 12.92 16.3799C12.87 16.5099 12.8 16.6099 12.71 16.7099C12.61 16.7999 12.5 16.8699 12.38 16.9199C12.26 16.9699 12.13 16.9999 12 16.9999Z" fill="#ff6467"/>
                            </svg>
                            <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.password.message}</span>
                        </p>
                    }


                            <div className={'flex items-center'}>
                                <div>
                                    <input {...register('image')} type={'file'} className={'hidden'} id={'file'}/>
                                    <label className={'cursor-pointer group '} htmlFor={'file'}>
                                        <svg width={90} className={'group-active:scale-125 transition-all duration-300'}
                                             xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                            <path
                                                d="M19,20H5c-1.657,0-3-1.343-3-3V7c0-1.657,1.343-3,3-3h14c1.657,0,3,1.343,3,3v10C22,18.657,20.657,20,19,20z"
                                                className={'fill-gray-100 group-hover:fill-indigo-100 transition-all duration-300'}/>
                                            <circle cx="17" cy="8" r="2"
                                                    className={'fill-gray-300 group-hover:fill-indigo-300 transition-all duration-300'}/>
                                            <path
                                                d="M4,16c0,0.548,0.448,1,1,1h14c0.552,0,1-0.444,1-1c0-1.364-1.688-3-2.5-3s-1.548,1-3.5,1c-2.442,0-4-5-6-5S4,13.362,4,16z"
                                                className={'fill-gray-300 group-hover:fill-indigo-300 transition-all duration-300'}/>
                                        </svg>
                                        <p className={'w-[100px] group-active:scale-125 group-hover:text-indigo-400 transition-all duration-300 text-sm ms-2 text-gray-400'}>انتخاب
                                            تصویر</p>
                                    </label>
                                </div>
                                <div>
                                    {prevImage && <img src={prevImage}  width={60} className={'rounded-xl'}/>}
                                </div>
                            </div>
                    {
                        errors.image &&
                        <p className={'flex items-center my-5'}>
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill="#ff6467"/>
                                <path d="M12 13.75C11.59 13.75 11.25 13.41 11.25 13V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V13C12.75 13.41 12.41 13.75 12 13.75Z" fill="#ff6467"/>
                                <path d="M12 16.9999C11.87 16.9999 11.74 16.9699 11.62 16.9199C11.5 16.8699 11.39 16.7999 11.29 16.7099C11.2 16.6099 11.13 16.5099 11.08 16.3799C11.03 16.2599 11 16.1299 11 15.9999C11 15.8699 11.03 15.7399 11.08 15.6199C11.13 15.4999 11.2 15.3899 11.29 15.2899C11.39 15.1999 11.5 15.1299 11.62 15.0799C11.86 14.9799 12.14 14.9799 12.38 15.0799C12.5 15.1299 12.61 15.1999 12.71 15.2899C12.8 15.3899 12.87 15.4999 12.92 15.6199C12.97 15.7399 13 15.8699 13 15.9999C13 16.1299 12.97 16.2599 12.92 16.3799C12.87 16.5099 12.8 16.6099 12.71 16.7099C12.61 16.7999 12.5 16.8699 12.38 16.9199C12.26 16.9699 12.13 16.9999 12 16.9999Z" fill="#ff6467"/>
                            </svg>
                            <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.image.message}</span>
                        </p>
                    }

                    <div className="field">
                       <button className={loading ? 'opacity-50 btt' : 'btt'} disabled={loading}>
                           {
                               loading ? <BeatLoader size={11}  speedMultiplier={0.7} color={'white'} />
                                   : 'ثبت نام'
                           }
                       </button>
                    </div>
                    <div className="signup-link">

                         ثبت نام کرده اید ؟ <Link href={'/login'}>وارد شوید </Link>
                    </div>
                </form>
            </div>

            <Toaster/>


        </div>

    );
};

export default Page;