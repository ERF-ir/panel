...
import {getCategory} from "@/services/requests/admin/postCateegory";
import {postValidation} from "@/services/validations/validation";
    ...
    return (
        ...
        <Master>
                ...
                <div className=" py-5 px-10 w-[85%]   flex items-center justify-between bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">
                    ...
                    <div>
                        <div className={'flex items-center'}>
                            ...
                            <h1 className={'text-gray-600 text-[1.1rem] ms-2 '}>افزودن پست جدید به سایت</h1>
                        </div>
            ...
            <div className={'flex justify-center mt-3 pb-16'}>
                <div className=" py-10 px-1 mt-5 w-[90%]    bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">

                    <form onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data" className={' ms-32 grid grid-cols-12 '}>

                        <div className={'col-span-6 my-10 '}>
                            <label className={'text-gray-500 block'}>عنوان پست :</label>
                            <input {...register("title")}
                                   type='text'  placeholder='...' className={'p-3.5 mt-3 w-[75%] ' +
                                ' shadow-[0_0_2px_#c9c9c9] focus:ring-2' +
                                ' ring-indigo-300  transition-all  text-sm text-gray-500 placeholder:text-sm' +
                                ' placeholder:text-gray-300 border border-gray-300 rounded-lg'} />
                        ...
                        <div className={'col-span-6 my-10 '}>
                            ...
                            <div className="select mt-3" >
                                <select {...register("category_id")}>
                                    ...
                                    {PostCategory
                                        .map(parenr => (
                                            <option value={parenr.id} key={parenr.id}>{parenr.name}</option>
                                        ))
                                    }
                                </select>
                            </div>
                            <p className={' p-2   rounded-full  flex items-center'}>
                                {errors.category_id && (
                                    <>
                                        ...
                                        <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.category_id.message}</span>
                                    </>
                                )}
                            </p>
                        ...
                        <div className={'col-span-12 mb-7'}>
                            <label className={'text-gray-500 block'}>خلاصه متن :</label>
                            <textarea rows={8} {...register("summery")}  placeholder='خلاصه ای از متن' className={'p-3.5 mt-3 w-[85%] ' +
                                ' shadow-[0_0_2px_#c9c9c9] focus:ring-2' +
                                ' ring-indigo-300  transition-all  text-sm text-gray-500 placeholder:text-sm' +
                                ' placeholder:text-gray-300 border border-gray-300 rounded-lg'} />
                            <p className={' p-2   rounded-full  flex items-center'}>
                                {errors.summery && (
                                    <>
                                        ...
                                        <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.summery.message}</span>
                                    </>
                                )}
                            </p>
                        </div>
                        ...
                        <div className={'col-span-12 '}>
                            <label className={'text-gray-500 block'}>متن اصلی :</label>
                            <textarea rows={11} {...register("body")}  placeholder='متن کامل' className={'p-3.5 mt-3 w-[85%] ' +
                                ' shadow-[0_0_2px_#c9c9c9] focus:ring-2' +
                                ' ring-indigo-300  transition-all  text-sm text-gray-500 placeholder:text-sm' +
                                ' placeholder:text-gray-300 border border-gray-300 rounded-lg'} />
                            <p className={' p-2   rounded-full  flex items-center'}>
                                {errors.body && (
                                    <>
                                        ...
                                        <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.body.message}</span>
                                    </>
                                )}
                            </p>
                        </div>
                       ...
                       <div className={'col-span-12 flex justify-between items-center'}>
                           <div className={''}>
                               <input  type={'file'} {...register('image')} className={'hidden'}  id={'file'}  />
                               <label className={'cursor-pointer group '} htmlFor={'file'} >
                                   <svg width={90} className={'group-active:scale-125 transition-all duration-300'} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                       <path d="M19,20H5c-1.657,0-3-1.343-3-3V7c0-1.657,1.343-3,3-3h14c1.657,0,3,1.343,3,3v10C22,18.657,20.657,20,19,20z" className={'fill-gray-100 group-hover:fill-indigo-100 transition-all duration-300'} />
                                       <circle cx="17" cy="8" r="2"  className={'fill-gray-300 group-hover:fill-indigo-300 transition-all duration-300'} />
                                       <path  d="M4,16c0,0.548,0.448,1,1,1h14c0.552,0,1-0.444,1-1c0-1.364-1.688-3-2.5-3s-1.548,1-3.5,1c-2.442,0-4-5-6-5S4,13.362,4,16z"  className={'fill-gray-300 group-hover:fill-indigo-300 transition-all duration-300'} />
                                   </svg>
                                   <p className={'w-[100px] group-active:scale-125 group-hover:text-indigo-400 transition-all duration-300 text-sm ms-2 text-gray-400'}>انتخاب تصویر</p>
                               </label>
                               <p className={' p-2   rounded-full  flex items-center'}>
                                   {errors.image && (
                                       <>
                                           ...
                                           <span className={'text-sm ms-2  bg-red-100 rounded-md py-2 px-3  text-red-400 '}>{errors.image.message}</span>
                                       </>
                                   )}
                               </p>
                           </div>
                           ...
                           {
                               Preview && <img src={Preview} width={100} className={'rounded-xl me-96'} alt={'fef'}/>
                           }


                           <div className={'me-[145px]'}>
