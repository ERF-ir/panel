...
import {deletePost, listPost, toggleStatus} from "@/services/requests/admin/post";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const page = () => {
    ...
    return (
      ...
      <Master>
          ...
          <div className=" py-5 px-10    flex items-center justify-between bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">
              ...
              <div>
                  <div className={'flex items-center'}>
                      ...
                      <h1 className={'text-gray-600 text-[1.1rem] ms-2 '}>لیست پست های سایت</h1>
                  </div>
          ...
          <div className=" py-10 px-14 mt-5 overflow-x-auto  bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">


              <button  disabled={isPending} onClick={()=>startTransition(()=>router.push('/content/post/store'))} className={'py-2.5  mb-5 space-x-2 flex ' +
                  ...
                  {isPending ? <BeatLoader size={8}  speedMultiplier={0.7} color={'#837cf3'} /> : (
                      <>
                          <span className={'text-sm'}>پست جدید</span>
                      </>
                  ) }
              </button>
              ...
              <table  className={'w-full border-collapse  text-center bo '}>
...

                  {
                      posts.map((post)=>(
                          ...
                          <tr key={post.id} className={'border-y *:py-4 even:bg-gray-50  ' }>
                              ...
                              <td>
                                  <label className="switch">
                                      <input defaultChecked={post.status} onChange={()=>changeStatus(post.id)}  type="checkbox"/>
                              ...
                              <td>
                                ...
                                <div className={'flex items-center  justify-center space-x-3 *:cursor-pointer'}>
                                    ...
                                    {loading[post.id] ? <BeatLoader size={8}  speedMultiplier={0.7} color={'#ff7476'} /> : (
                                        <>
                                            <svg  onClick={()=>handleDelete(post.id)} width="35"  className={'border active:bg-red-200 transition-all hover:bg-red-50 p-1.5 *:fill-red-500 border-red-500 rounded-xl'} height="35"  viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <Dialog>
                                        <DialogTrigger>
                                            <svg width="35"  className={'border active:bg-sky-200 transition-all hover:bg-sky-50 p-1.5 *:fill-sky-500 border-sky-500 rounded-xl'} height="35" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <DialogContent  >
                                            <DialogHeader>
                                                <DialogTitle className={'text-center text-gray-600'} >{post.title}</DialogTitle>
                                            </DialogHeader>
                                            <div className={'rounded-lg flex justify-center'}>
                                                <img alt={'d'}  width={300} src={post.image}  />

                                            </div>
                                            <h1>خلاصه متن : </h1>
                                            <p className={'text-right max-w-[450px] break-words'}>
                                                {post.summery}
                                            </p>
                                            <h1>متن اصلی : </h1>
                                            <p className={'text-right max-w-[450px] break-words'}>
              ...
              {posts.length ? '': <div className={'flex justify-center mt-10'}><BeatLoader size={17}   speedMultiplier={0.9} color={'#74dfff'}/></div>}
