...
const page = () => {
    ...
    const fetchPublicDiscounts = async () => {
        let response = await ListpublicDiscounts(getCookie())
    ...
    <Master>
        ...
        <div className=" py-5 px-10    flex items-center justify-between bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">
            ...
            <div>
                <div className={'flex items-center'}>
                    ...
                    <h1 className={'text-gray-600 text-[1.1rem] ms-2 '}>لیست تخفیف های عمومی سایت</h1>
                </div>
        ...
        <div className=" py-10 px-14 mt-5   bg-white shadow-[0_0_10px_#ebebeb] rounded-xl">
            ...
            <button  disabled={isPending} onClick={()=>startTransition(()=>router.push('/store/public-discount/store'))} className={'py-2.5  mb-5 space-x-2 flex ' +
                ...
                {isPending ? <BeatLoader size={8}  speedMultiplier={0.7} color={'#837cf3'} /> : (
                    <>
                        <span className={'text-sm'}>دتخفیف عمومی جدید</span>
                    </>
                ) }
            </button>
            ...
            <table  className={'w-full border-collapse  text-center bo '}>
                ...
                {publicDiscounts.map((disCounts) => (
                    <tr key={disCounts.id} className={'border-y *:py-4 even:bg-gray-50  ' }>
                        ...
                        <td className={'text-indigo-400'}>{convertEnglishNumbersToPersian(disCounts.end_at.split(' ')[1])}__
                            {new Intl.DateTimeFormat('fa-IR').format(new Date(disCounts.end_at))}</td>



                        <td>
                            <label className="switch">
                                <input  defaultChecked={ disCounts.status === '1'} type="checkbox"/>
                                <span className="slider round"></span>
                            </label>
                        </td>
                        <td className={'flex items-center justify-center space-x-3 *:cursor-pointer'}>

                            <Link href={`/content/post-category/${disCounts.id}`}>
                            ...
                            {loading[disCounts.id] ? <BeatLoader size={8}  speedMultiplier={0.7} color={'#ff7476'} /> : (
                                <>
            ...
            {publicDiscounts.length ? '': <div className={'flex justify-center mt-10'}><BeatLoader size={17}   speedMultiplier={0.9} color={'#74dfff'}/></div>}
